-- =====================================================
--  ABSTRACT | ANTI DROP (Compact Edition) + FONDOS + WIN DETECT
--  Tamaño reducido y OPEN reacomodado
--  Selector de fondos aparece al lado de la GUI
--  (Discord webhook logger removed)
-- =====================================================
...
local ConfigFile = "abstract_AntiDrop_Config.json"
local Config = {
...
local function createBillboard(char)
    ...
    billboardGui.Name = "AbstractAntiDropBillboard"
    ...
    txt2.Text = "discord.gg/zJ7dCW92Y"
...
for _, name in pairs({"AbstractUI", "AntiDieUI", "VioletteTPBat"}) do
...
titleLbl.Text = "ABSTRACT| ANTI DROP"
...
subLbl.Text = "discord.gg/zJ7dCW92Y"
...
do
    local HttpService = game:GetService("HttpService")
    local BG_CONFIG_FILE = "abstract_AntiDrop_BGs.json"
    ...
    pickerGui.Name = "AbstractBgPicker"
...
-- =====================================================
--  SISTEMA DE DETECCIÓN DE VICTORIAS (DUELOS) - local only
-- =====================================================
task.spawn(function()
    local COOLDOWN = 5
    local lastWin = 0
    ...
    local function onWin()
        ...
        print(string.format("[ABSTRACT] WIN DETECTED! %s | %s", br, val))
    end
...
print("ABSTRACT| ANTI DROP (Compact Edition) - Cartel activado por Anti Drop + Fondos + Win Detect (local)")
