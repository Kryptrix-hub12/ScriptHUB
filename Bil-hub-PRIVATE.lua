...
local function recalculateScale()
    ...
    local newScale = 1
    if isMobile then
        newScale = math.clamp(h / 1050, 0.38, 0.54)
    else
        newScale = math.clamp(h / 850, 0.62, 0.94)
    end
...
local gui_sg = Instance.new("ScreenGui"); gui_sg.Name = "AidenHub_V2"; gui_sg.ResetOnSpawn = false; gui_sg.IgnoreGuiInset = true; gui_sg.DisplayOrder = 9999999; gui_sg.Parent = ((gethui and gethui()) or game:GetService("CoreGui"))
...
do
    ...
    barra.Name = "BilTopBar"
    barra.Size = UDim2.new(0, 240, 0, 46)
    barra.Position = UDim2.new(0.5, -120, 1, -120)
    ...
    nHub.Parent = nome
    _tituloGrad(nHub, {
        ColorSequenceKeypoint.new(0.00, Color3.fromRGB(180,  60,   0)),
        ColorSequenceKeypoint.new(0.30, Color3.fromRGB(255, 140,   0)),
        ColorSequenceKeypoint.new(0.50, Color3.fromRGB(255, 220, 120)),
        ColorSequenceKeypoint.new(0.70, Color3.fromRGB(255, 140,   0)),
    ...
    clique.MouseButton1Click:Connect(function()
        if PrivateServerCode and PrivateServerCode ~= "" then
            ...
            aviso.TextColor3 = Color3.fromRGB( 96, 175, 255)
            task.spawn(function()
                local ok = pcall(function()
...
-- BIL_PAID_MAIN_LAYOUT: compact glass hub near the bottom on PC + mobile
pcall(function()
    if main and main.Parent then
        main.Size = UDim2.new(0,330,0,420)
        main.Position = UDim2.new(0.5,-165,1,-445)
    end
end)
...
task.spawn(function()
    ...
    if plots then
        ...
        plots.ChildAdded:Connect(function(p) task.defer(setupPlotListener, p, 0) end)
    ...
    task.spawn(function() while true do SharedState.AllAnimalsCache=allAnimalsCache; task.wait(0.05) end end)
end)
player:GetAttributeChangedSignal("Stealing"):Connect(function()
...
local function _AidenRefreshTarget(force)
    ...
    if not force and _AidenFastTargetMode == mode and (now - _AidenFastTargetAt) < 0.01 then
...
function makeHeader(f,t,isMain)
    local bar=Instance.new("Frame"); bar.Name="BilBar"; bar.Size=UDim2.new(1,0,0,2)
    bar.BackgroundColor3=(BIL and BIL.GRAD_A) or Theme.Accent; bar.BorderSizePixel=0; bar.ZIndex=5; bar.Parent=f
    local g=Instance.new("UIGradient"); g.Rotation=0
    g.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,(BIL and BIL.GRAD_A) or Theme.Accent),
                               ColorSequenceKeypoint.new(1,(BIL and BIL.GRAD_B) or Theme.AccentLight)}); g.Parent=bar
    ...
    if isMain then
        ...
        l.Font=Enum.Font.GothamBlack; l.TextSize=14; l.TextXAlignment=Enum.TextXAlignment.Left; l.Parent=h
    else
...
function makeSyncStateRow(parent,text,toggleName,callback,bindName)
    ...
    if bindName then
        local kb=Instance.new("TextButton"); kb.Name="BilBind"; kb.Size=UDim2.new(0,56,0,22)
...
function makeSyncMainToggle(parent,text,toggleName,callback,bindName)
    ...
    if bindName then
        local kb=Instance.new("TextButton"); kb.Name="BilBind"; kb.Size=UDim2.new(0,56,0,21)
...
main,mainBody=makeMainPanel("Aiden Hub V2",UDim2.new(0,330,0,420),UDim2.new(0.5,-165,1,-445))
...
function loadTab(tabName)
    ...
    elseif tabName=="UI" then
        if _G.BilExtras then
            makeSyncMainToggle(mainBody,"Face Away","Face Away",function(on) _G.BilExtras.setFaceAway(on) end)
            makeSyncMainToggle(mainBody,"Face Nearest Player","Face Nearest",function(on) _G.BilExtras.setFaceNearest(on) end,"Face Nearest")
        end
    ...
    elseif tabName=="Misc" then
        if _G.BilExtras then
            makeSyncMainToggle(mainBody,"Anti Colisao (atravessar players)","Anti Colisao",function(on) _G.BilExtras.setAntiColisao(on) end)
            makeSyncMainToggle(mainBody,"Anti Flasher (remove acessorios)","Anti Flasher",function(on) _G.BilExtras.setAntiFlasher(on) end)
            makeSyncMainToggle(mainBody,"Ps On Steal","Ps On Steal",function(on) _G.BilExtras.setPs(on) end)
        end
...
UIS.InputBegan:Connect(function(input,gp) if gp then return end
    ...
    local actions={
        ...
        ["Face Nearest"]=function()
            if _G.BilExtras then
                _G.BilExtras.setFaceNearest(not Config.FaceAwayNearest)
            end
        end,
...
-- BIL_INFINITE_JUMP: hold Space to repeatedly jump
local BIL_INFINITE_JUMP = true
local BIL_INF_JUMP_SPEED = 55

UIS.JumpRequest:Connect(function()
    if not BIL_INFINITE_JUMP then return end
    local char = LP.Character
    if not char then return end
    local root = char:FindFirstChild("HumanoidRootPart")
    if root then
        root.Velocity = Vector3.new(root.Velocity.X, BIL_INF_JUMP_SPEED, root.Velocity.Z)
    end
end)
...
do
    ...
    local function construirBotaoPs()
        ...
        bar.BackgroundColor3 = (BIL and BIL.GRAD_A) or Theme.Accent
        bar.BorderSizePixel = 0
        bar.ZIndex = 3
        bar.Parent = psBtn
        local g = Instance.new("UIGradient")
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, (BIL and BIL.GRAD_A) or Theme.Accent),
            ColorSequenceKeypoint.new(1, (BIL and BIL.GRAD_B) or Theme.AccentLight),
        })
        g.Parent = bar
        ...
        UIS.InputEnded:Connect(function(i)
            ...
            or i.UserInputType == Enum.UserInputType.Touch then
                if arrastando and moveu then
                    pcall(function() rememberPosition("BIL Ps Button", psBtn) end)
                end
                arrastando = false
            end
        end)
        click.MouseButton1Click:Connect(function()
            ...
            if _G.BilExtras then _G.BilExtras.setPs(not Config.PsOnSteal) end
        end)
        pcall(function() applySavedPosition("BIL Ps Button", psBtn) end)
    ...
    local function marcaGarantir()
        ...
        mkFolder = Instance.new("Folder"); mkFolder.Name = "BilFaceAway"; mkFolder.Parent = cam
    ...
    local function construirPainel()
        ...
        painel, corpo = makeQuickPanel("BIL\nPanel Face Away",
        ...
        pcall(function() applySavedPosition("BIL\nPanel Face Away", painel) end)
    end
    local ultimoPaint = 0
