-- =====================================================================
--discord.gg/sabcom script for freee at discord.gg/sabcom
--discord.gg/sabcom script for freee at discord.gg/sabcom
--discord.gg/sabcom script for freee at discord.gg/sabcom
--discord.gg/sabcom script for freee at discord.gg/sabcom
--discord.gg/sabcom script for freee at discord.gg/sabcom
...
-- every module function. Diagnostic in _G.SabcomSyncDiag.
local _xchan
...
local function _chans()
    ...
    if found then
        _xchan = found
        _G.SabcomSyncDiag = string.format("bypassed via %s - %d channels (attempt %d)",
            tostring(where), n, _attempts)
    elseif _attempts >= MAX_ATTEMPTS then
        _G.SabcomSyncDiag = "channels NOT found after " .. MAX_ATTEMPTS
...
_G.Sabcom_GetPlotChannel=function(plotName)return _G.SabcomSyncGet(plotName)end
    ...
    while (os.clock() - warmT0) < 15 do
        ...
        for _, plot in ipairs(kids) do
            if not done[plot.Name] then
                -- por Order ("PlotN"), com o UUID de reserva. Usando so o
                -- UUID este pre-aquecimento falhava em TODAS as bases e
                -- girava os 15s inteiros sem aquecer nada -- os dados so
                -- chegavam depois, no scan comum, e por isso demorava.
                local ch
...
do
  if not _G.__sabcomNetBuilt then
    _G.__sabcomNetBuilt = true
    ...
    local function build()
        ...
        if not getgc then
            warn("[sabcom Net] build() FAILED - getgc not available")
            return false
        end
        ...
        if not a then
            warn("[sabcom Net] build() FAILED - no hashing functions found.")
            warn("[sabcom Net] Source matches:", _srcMatches, "Samples:", table.concat(_srcSamples, ", "))
            pcall(function()
                local children = netFolder:GetChildren()
                warn("[sabcom Net] Net folder has", #children, "children. First few:")
        ...
        if #cands == 1 then secret = cands[1]; print("[sabcom Net] build() OK (single candidate)"); return true end
        for _, sec in ipairs(cands) do
            local ok, h = pcall(function() return F1(_bufFromString(cipher), _bufFromString(sec .. jobId)) end)
            if ok and type(h) == "string" and netFolder:FindFirstChild("RE/" .. h) then secret = sec; print("[sabcom Net] build() OK (multi candidate, resolved)"); return true end
        end
        warn("[sabcom Net] build() FAILED - found", #cands, "GUID candidates but none resolved a valid remote.")
        pcall(function()
            local children = netFolder:GetChildren()
            warn("[sabcom Net] Net folder has", #children, "children. First few:")
            for i = 1, math.min(5, #children) do
    ...
    local function resolve(prefix, name)
        ...
        if not (F1 and secret) and not build() then
            if not _resolveWarned[name] then
                ...
                warn("[sabcom Net] resolve('" .. name .. "') FAILED - build() failed and no unhashed '" .. prefix .. "/" .. name .. "' found.")
            end
            return nil
        end
        local jobId = game.JobId
        ...
        if not ok or type(h) ~= "string" then
            if not _resolveWarned[name] then
                _resolveWarned[name] = true
                warn("[sabcom Net] resolve('" .. name .. "') FAILED - hash error:", tostring(h))
            end
            return nil
        end
        ...
        if not remote then
            if not _resolveWarned[name] then
                _resolveWarned[name] = true
                warn("[sabcom Net] resolve('" .. name .. "') FAILED - remote '" .. prefix .. "/" .. h .. "' not found in Net folder.")
            end
            return nil
        end
        return remote
...
_G.Net = {
  ...
  end -- closes if not __sabcomNetBuilt
end -- closes do

-- Always patch _G.Net with GetRemote/FireServer (survives re-exec when __sabcomNetBuilt is already true)
do
...
-- with _G.Sabcom_FPS_CAP before load (0 = unlimited, or a number to cap).
_G.Sabcom_FPS_CAP = _G.Sabcom_FPS_CAP or 999
do
    local function applyCap()
        if not setfpscap then return end
        pcall(setfpscap, tonumber(_G.Sabcom_FPS_CAP) or 999)
    end
    applyCap()
    task.spawn(function()
        while true do
...
-- have a known-good GUID, set `_G.Sabcom_RESET_GUID` before the script runs.
...
GUID = GUID or _G.Sabcom_RESET_GUID or _newGUID()
resetRemote = nil
...
--   _G.Sabcom_RESET_REMOTE = <a RemoteEvent instance>      -- direct, or
--   _G.Sabcom_RESET_REMOTE = "PlaintextRemoteName"          -- resolved via _G.Net
if _G.Sabcom_RESET_REMOTE ~= nil then
    if typeof(_G.Sabcom_RESET_REMOTE) == "Instance" then
        resetRemote = _G.Sabcom_RESET_REMOTE
    elseif type(_G.Sabcom_RESET_REMOTE) == "string" and _G.Net then
        pcall(function()
            resetRemote = (_G.Net.GetRemote and _G.Net:GetRemote(_G.Sabcom_RESET_REMOTE))
                or _G.Net:RemoteEvent(_G.Sabcom_RESET_REMOTE)
        end)
    end
end
...
-- =====================================================================
_G.SabcomGrappleValue = _G.SabcomGrappleValue or 0.33  -- live-tunable grapple distance value
_G.__sabcomUseItemPayload = _G.__sabcomUseItemPayload or nil
do
    local RS = game:GetService("ReplicatedStorage")
    local netFolder = RS:WaitForChild("Packages"):WaitForChild("Net")
    local PAYLOAD_FILE = "sabcom_grapple_payload.txt"
    ...
    _G.SabcomFindUseItem = findUseItemByDualHash
    ...
    _G.SabcomResolveHashed = resolveHashed
    ...
    -- _G.Sabcom_GRAPPLE_PAYLOAD = { [1]=<num>, n=1 } to override without a file.
    local function loadPersistedPayload()
        if _G.Sabcom_GRAPPLE_PAYLOAD then return _G.Sabcom_GRAPPLE_PAYLOAD end
    ...
    _G.__sabcomUseItemPayload = loadPersistedPayload()
    ...
    task.spawn(function()
        ...
        while os.clock() - t0 < 60 do
            if findUseItemByDualHash() then
                warn("[sabcom] UseItem resolved: " .. _G.SabcomFindUseItem().Name)
                return
            end
            task.wait(0.25)
        end
    ...
    -- ============================================================
    --  O DISPARO EM SI  (compartilhado pelos dois modos de mira)
    --
    --  Escreve a mira nas instancias do PlayerMouse E gira a camera,
    --  chama Activate(), e SEGURA o estado por 4 frames antes de
    --  devolver.
    --
    --  Esses 4 frames sao o ponto critico: SignalBehavior = Deferred
    --  e o padrao do Roblox, entao Activate() so ENFILEIRA o handler
    --  -- ele roda no fim do frame. Restaurando na linha seguinte, o
    --  script do item lia o estado ja revertido e mirava no cursor.
    -- ============================================================
    _G.__sabcomGrappleDisparar = function(tool, pos, alvo, char, RSv)
        local refs = _G.__sabcomPMRefs
        if not refs then
            refs = {}
            local vistos = {}
    ...
    _G.SabcomFireGrapple2 = function(value, exato)
        ...
        if not r then
            if not _G.__sabcomGrappleWarned then
                _G.__sabcomGrappleWarned = true
                warn("[sabcom] Grapple remote unresolved (Net folder still streaming?)")
            end
            return false
        end
        -- ============================================================
        ...
        --  sabcomGrappleSubirEHook. Com ele removido, ninguem mais
    ...
    _G.SabcomGrappleReady = function() return resolveHashed("UseItem", "RemoteEvent") ~= nil end

    -- ============================================================
    --  O CICLO
    --
    --     grapple na direcao da base  ->  TP
    --
    --  Sem pulo e sem hook pra baixo: aquilo foi removido a pedido.
    --
    --  Ponto unico: TP-on-load e o Manual TP (CapsLock) chamam esta
    --  mesma funcao, entao os dois se comportam igual e so existe um
    --  lugar pra ajustar.
    --
    --  A espera entre o hook e o TP e de 2 frames -- o minimo util.
    --  Nao pode ser zero: Activate() so ENFILEIRA o handler (o Roblox
    --  usa SignalBehavior = Deferred), entao no frame do disparo o
    --  script do item ainda nem rodou e o puxao nao comecou.
    -- ============================================================
    _G.SabcomCicloTP = function(esperaMax)
        if _G.__sabcomCicloRodando then return false end
        _G.__sabcomCicloRodando = true

        local RSvc = game:GetService("RunService")
        local eu = game:GetService("Players").LocalPlayer

        -- ========================================================
        --  ALVO PRIMEIRO. SEMPRE.
        --
        --  Era isto que fazia "as primeiras 10 vezes falharem e
        --  depois funcionar sempre": nos segundos iniciais o cache
        --  de pets ainda esta vazio, entao o ciclo subia, gastava o
        --  hook, e o TP entrava sem ter pra onde ir. Passados uns
        --  20-30s o scanner popula e nunca mais erra -- exatamente o
        --  padrao que voce descreveu.
        --
        --  Agora confere antes: sem alvo, nem sobe. O hook nao e
        --  desperdicado e voce recebe o motivo no console.
        -- ========================================================
        ...
        if not temAlvo() then
            _G.__sabcomCicloRodando = false
            warn(("[sabcom] ciclo TP: nenhum alvo no cache apos %.1fs -- "
        ...
        -- ========================================================
        --  STREAMING DO DESTINO -- a causa do rollback inicial
        --
        --  Com StreamingEnabled, a regiao da base alvo ainda nao foi
        --  transmitida pro cliente nos primeiros segundos. Voar pra
        --  la significa atravessar espaco que, pro seu cliente, esta
        --  vazio -- e o servidor rejeita o movimento e te puxa de
        --  volta. Depois que o mapa carrega, para de acontecer: e
        --  exatamente o "rollback insano nas primeiras vezes e
        --  depois TP a vontade".
        --
        --  RequestStreamAroundAsync pede a regiao e ESPERA ela
        --  chegar. O hub ja fazia isso no load (linha ~324), mas
        --  aquilo roda uma vez e nao termina a tempo dos primeiros
        --  TPs. Aqui e sob demanda, no destino, logo antes de sair.
        -- ========================================================
        -- posicao da base alvo: serve pro streaming E pra direcao do hook
        local destino
        do
        ...
        -- ========================================================
        --  GRAPPLE NA DIRECAO DA BASE, DO CHAO MESMO
        --
        --  Sem pulo e sem hook pra baixo -- aquilo foi removido.
        --
        --  Passando o destino sem o segundo argumento, o
        --  sabcomFireGrapple2 entra no modo de BUSCA: pega a direcao
        --  horizontal ate a base e procura chao a 45, 38, 31, 25,
        --  20, 16 e 12 studs, do mais longe pro mais perto. O
        --  primeiro que cair na faixa de 10-50 studs que o item
        --  aceita vira o alvo do hook.
        --
        --  Ou seja: puxa voce na direcao da base, rente ao chao.
        -- ========================================================
        local ok = false
        if destino and _G.SabcomFireGrapple2 then
            pcall(function() ok = _G.SabcomFireGrapple2(destino) and true or false end)
        end
        if _G.__LMARK then _G.__LMARK("hook na direcao da base: " .. tostring(ok)) end

        -- Espera entre o hook e o TP.
        --
        -- Nao pode ser zero: Activate() so ENFILEIRA o handler
        -- (SignalBehavior = Deferred). Com zero, o TP comeca no mesmo
        -- frame e a primeira coisa que ele faz e equipar o tapete --
        -- o que TIRA a Grapple Hook da mao. Quando o handler enfim
        -- roda, o script do item testa Tool.Name == "Grapple Hook",
        -- nao encontra, e sai sem disparar.
        local ESPERA_HOOK_TP = 0.12
        task.wait(ESPERA_HOOK_TP)

        if _G.SabcomStartSideTP then
            task.spawn(_G.SabcomStartSideTP)
        elseif _G.Sabcom_ExecuteManualTP then
...
-- LAZY-LOAD QUEUE
-- =====================================================================
_G.__sabcomLazyQ = _G.__sabcomLazyQ or {}
local function LazyInit(name, fn)
    table.insert(_G.__sabcomLazyQ, { name = name, fn = LPH_NO_VIRTUALIZE(fn) })
end
task.defer(LPH_NO_VIRTUALIZE(function()
    while true do
        if #_G.__sabcomLazyQ > 0 then
            local item = table.remove(_G.__sabcomLazyQ, 1)
            local ok, err = pcall(item.fn)
            if not ok then warn("[sabcom Lazy]", item.name, err) end
            task.wait(0.08)
        else
            task.wait(0.5)
        end
...
_G._sabcomPanelVis = _G._sabcomPanelVis or {}
    ...
    for _, item in ipairs(_G.lazyUIs) do
        if item.element and not item.cancelled then
            ...
            if item.panelName then
                ...
                local fromG = _G._sabcomPanelVis[item.panelName]
                if fromG ~= nil then
                    vis = fromG
    ...
    for sg, master in pairs(scaledGuis) do
        pcall(function()
            if sg and sg.Parent and master and master.Parent then
                local scaleObj = master:FindFirstChild("sabcom_GlobalScale")
                if scaleObj then
...
local function registerScreenGui(sg)
    local master = sg:FindFirstChild("sabcom_MasterFrame")
    if not master then
        master = Instance.new("Frame")
        master.Name = "sabcom_MasterFrame"
        master.BackgroundTransparency = 1
        master.BorderSizePixel = 0
        master.Parent = sg
        
        local scaleObj = Instance.new("UIScale")
        scaleObj.Name = "sabcom_GlobalScale"
...
local old = playerGui:FindFirstChild("sabcomHub_V3"); if old then old:Destroy() end
local gui_sg = Instance.new("ScreenGui"); gui_sg.Name = "sabcomHub_V3"; gui_sg.ResetOnSpawn = false; gui_sg.IgnoreGuiInset = true; gui_sg.DisplayOrder = 9999999; gui_sg.Parent = playerGui
local gui = registerScreenGui(gui_sg)

-- ============================================================
--  BARRA DO TOPO  --  sabcom Hub | FPS/PING | discord
--
--  Substitui o botao "JP" de 48x28 que existia aqui. A funcao de
--  entrar no servidor privado continua: agora e o clique na barra
--  inteira, entao nao se perde nada.
--
--  As cores saem do Theme, entao ela acompanha o resto do hub em
--  vez de ter preto e cinza chumbados como o botao antigo tinha.
-- ============================================================
do
    local jpSg = Instance.new("ScreenGui")
    jpSg.Name = "sabcomHub_TopBar"
    ...
    barra.Name = "sabcomTopBar"
    barra.Size = UDim2.new(0, 240, 0, 46)
    barra.Position = UDim2.new(0.5, -120, 0, 8)
    barra.BackgroundColor3 = Color3.fromRGB(19, 18, 24)
    ...
    -- faixa de acento no topo, a mesma assinatura dos paineis
    local faixa = Instance.new("Frame")
    faixa.Size = UDim2.new(1, 0, 0, 2)
    faixa.BackgroundColor3 = Color3.fromRGB(139, 92, 246)
    ...
    g.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(1, Color3.fromRGB(232, 84, 168)),
    })
    g.Parent = faixa
    ...
    nome.Text = "sabcom HUB"
    nome.TextColor3 = Color3.fromRGB(240, 239, 246)
    nome.Font = Enum.Font.GothamBlack
    nome.TextSize = 13
    nome.Parent = barra

    -- FPS e PING: o contador ja existente escreve aqui, entao esta
    -- label e exportada como fpsText
    local stats = Instance.new("TextLabel")
    stats.Size = UDim2.new(0.55, 0, 0, 12)
    stats.Position = UDim2.new(0, 5, 0, 22)
    stats.BackgroundTransparency = 1
    stats.Text = "FPS: -- | PING: --"
    stats.TextColor3 = Color3.fromRGB(178, 142, 255)
    ...
    disc.Text = "discord.gg/sabcom"
    disc.TextColor3 = Color3.fromRGB(134, 131, 152)
    ...
    aviso.Text = ""
    aviso.TextColor3 = Color3.fromRGB(242, 92, 114)
    aviso.Font = Enum.Font.GothamMedium
    aviso.TextSize = 9
    aviso.Parent = barra

    -- a barra inteira e o botao de entrar no servidor privado, que
    -- era o que o "JP" fazia
    ...
    clique.MouseEnter:Connect(function() barra.BackgroundColor3 = Color3.fromRGB(46, 43, 57) end)
    clique.MouseLeave:Connect(function() barra.BackgroundColor3 = Color3.fromRGB(19, 18, 24) end)
    ...
    clique.MouseButton1Click:Connect(function()
        if PrivateServerCode and PrivateServerCode ~= "" then
            ...
            aviso.TextColor3 = Color3.fromRGB(178, 142, 255)
            task.spawn(function()
                local ok = pcall(function()
    ...
    -- entrega a label pro contador de FPS/PING que roda mais abaixo
    _G.__sabcomStats = stats
end
...
-- ============================================================
--  THEME - sabcom
--  Profundidade por camada, nao por borda: cada nivel clareia um
--  degrau (MainBackground < Background < Panel < Row < RowHover).
--  O acento e raro de proposito - so foco, selecao e scrollbar.
--  As CHAVES sao as mesmas: applyTheme() casa por chave e o hub le
--  Theme.X em centenas de lugares. Mudar valor e seguro, mudar
--  nome de chave quebraria tudo.
-- ============================================================
Themes = {
    Light = {
        Background=Color3.fromRGB(250,249,252), MainBackground=Color3.fromRGB(243,242,247),
        Panel=Color3.fromRGB(236,234,242), Row=Color3.fromRGB(228,226,236), RowHover=Color3.fromRGB(216,213,228),
        Accent=Color3.fromRGB(124,58,237), AccentLight=Color3.fromRGB(146,88,245),
        Green=Color3.fromRGB(22,163,94), Red=Color3.fromRGB(220,54,84), Red2=Color3.fromRGB(178,34,64),
        Text=Color3.fromRGB(24,22,32), Dim=Color3.fromRGB(122,118,140), Stroke=Color3.fromRGB(206,202,218),
        SoftButton=Color3.fromRGB(232,230,240), SoftButtonHover=Color3.fromRGB(220,217,232),
        SoftAccent=Color3.fromRGB(232,230,240), SoftAccentHover=Color3.fromRGB(220,217,232),
        ToggleOff=Color3.fromRGB(214,211,224), ToggleOff2=Color3.fromRGB(214,211,224),
        InputBg=Color3.fromRGB(255,255,255), SliderBg=Color3.fromRGB(206,202,218),
        BlacklistHover=Color3.fromRGB(240,206,214), BlacklistLeave=Color3.fromRGB(228,226,236),
    },
    Dark = {
        Background=Color3.fromRGB(19,18,24), MainBackground=Color3.fromRGB(13,12,17),
        Panel=Color3.fromRGB(27,26,34), Row=Color3.fromRGB(33,31,41), RowHover=Color3.fromRGB(46,43,57),
        Accent=Color3.fromRGB(139,92,246), AccentLight=Color3.fromRGB(178,142,255),
        Green=Color3.fromRGB(74,222,138), Red=Color3.fromRGB(242,92,114), Red2=Color3.fromRGB(198,48,82),
        Text=Color3.fromRGB(240,239,246), Dim=Color3.fromRGB(134,131,152), Stroke=Color3.fromRGB(58,55,73),
        SoftButton=Color3.fromRGB(35,33,44), SoftButtonHover=Color3.fromRGB(49,46,61),
        SoftAccent=Color3.fromRGB(35,33,44), SoftAccentHover=Color3.fromRGB(49,46,61),
        ToggleOff=Color3.fromRGB(43,41,53), ToggleOff2=Color3.fromRGB(43,41,53),
        InputBg=Color3.fromRGB(24,23,30), SliderBg=Color3.fromRGB(56,53,70),
        BlacklistHover=Color3.fromRGB(92,48,66), BlacklistLeave=Color3.fromRGB(49,46,61),
    }
}

sabcom = { NOME="sabcom", GRAD_A=Color3.fromRGB(139,92,246), GRAD_B=Color3.fromRGB(232,84,168) }
...
function applyTheme(themeName)
    ...
    local function updateBottomBarTheme()
        ...
        for _, child in ipairs(bottomBar:GetChildren()) do
            ...
            elseif child:IsA("TextLabel") then
                if child.Text == "|" or child.Text == "discord.gg/sabcom" then
                    child.TextColor3 = toTheme.AccentLight
                elseif child.Text == "By:discord.gg/sabcom" then
    ...
    pcall(function()
        local sg = playerGui:FindFirstChild("sabcomHub_V3")
    ...
    for _, name in ipairs({"sabcom_RemoteSell", "sabcom_StealProgressBar", "XiAdminPanel"}) do
        pcall(function()
...
-- CONFIG (merged)
local CONFIG_FILE = "sabcom_hub_v3_config.json"
local PS_CODE_FILE = "sabcom_hub_pscode.txt"
PrivateServerCode = ""
...
Config = {
    ...
    -- Priority e o modo principal. Vinha StealHighest=true, e como nao
    -- existe linha "Steal Highest" em painel nenhum, uma instalacao nova
    -- caia em Highest com os dois toggles visiveis marcados OFF -- e o
    -- unico jeito de sair era ligar e desligar o Nearest.
...
do
_G.stealthGet=function(n) return _G.SabcomSyncGet(n) end
...
-- =====================================================================
_G.SabcomGoodBoys = _G.SabcomGoodBoys or {}
_G.SabcomBadBoys  = _G.SabcomBadBoys or {}
do
    local URL = "nil"
    ...
    local CACHE_FILE = "sabcom_fmly_boys.json"   -- persisted across joins (executor filesystem)
    ...
    local function applyArrays(goodArr, badArr)
        ...
        _G.SabcomGoodBoys, _G.SabcomBadBoys = good, bad
...
_G.SabcomIsGoodBoy = function(plr) return plr ~= nil and _G.SabcomGoodBoys[tostring(plr.Name):lower()] == true end
_G.SabcomIsBadBoy  = function(plr) return plr ~= nil and _G.SabcomBadBoys[tostring(plr.Name):lower()] == true end

local function isPlayerBlacklisted(plr)
    if not plr then return false end
    -- FMLY good boys are always protected -- block every admin command on them.
    if _G.SabcomGoodBoys[tostring(plr.Name):lower()] then return true end
...
local function getPlotOwner(plot)
    ...
    if Synchronizer then
        local ch = _G.Sabcom_GetPlotChannel(plot.Name)
        if ch then
...
local CURSED_RESET_GUID = _G.Sabcom_RESET_GUID or "f888ee6e-c86d-46e1-93d7-0639d6635d42"
...
-- or set _G.Sabcom_RESET_REMOTE (a RemoteEvent) before running to pin it.
if typeof(_G.Sabcom_RESET_REMOTE) == "Instance" then cursedResetRemote = _G.Sabcom_RESET_REMOTE end
...
-- ============================================================
--  INSTANT RESET  (metodo do RESET.txt)
--
--  Burst de velocidade + Health=0 por 0.15s, e depois do respawn:
--  equipa o carpete, teleporta pras coordenadas e pula.
--
--  Substituiu o metodo antigo, que dependia de capturar um remote
--  do jogo (cursedResetRemote) e spammar payload ate o personagem
--  trocar -- se a captura falhasse, o reset simplesmente nao vinha.
-- ============================================================
...
local function instantReset()
    -- DISPARO UNICO. Duas travas, porque uma so nao bastava:
    --   1. instaResetCooldown -- barra chamada durante a sequencia
    --   2. _ultimoReset       -- barra o repique logo depois dela
    -- O Esc do jogo, a tecla e o botao da aba Actions chamam todos
    -- esta mesma funcao; sem a segunda trava, dois desses caminhos
    -- disparando quase juntos encadeiam um reset atras do outro e
    -- parece que o recurso ficou ligado.
    ...
    -- TOKEN DE DONO. instaResetCooldown e global e toda limpeza o zerava
    -- as cegas: a rede de 12s de um reset antigo apagava a trava de um
    -- reset NOVO que ja estava rodando. Com o token, cada execucao so
    -- limpa o que e dela.
    local meuTurno = {}
    _G.__resetDono = meuTurno

    -- devolve 16 em qualquer leitura de WalkSpeed enquanto o burst roda
    local mt, oldIdx, minhaClosure
    local destravou = false          -- passei setreadonly(mt,false)?
    ...
    -- Se o pcall morreu DEPOIS do setreadonly(mt,false) -- newcclosure
    -- ausente, por exemplo -- a metatable do game ficava destravada pro
    -- resto da sessao. Isso e instabilidade e, pior, estado trivial de
    -- detectar. Retranca aqui, independente do hook ter dado certo.
    ...
    local function desfazerHook()
        ...
        pcall(function()
            setreadonly(mt, false)
            -- SO restaura se a closure no slot ainda for a minha. Dois
            -- resets sobrepostos encadeavam (H2 captura H1 como oldIdx) e
            -- a restauracao fora de ordem deixava uma closure morta como
            -- __index do game -- ou seja, TODA leitura de propriedade de
            -- TODA Instance passando por Lua pelo resto da sessao.
    ...
    -- so limpa a trava se ela ainda for minha
    ...
    conn = RunService.Heartbeat:Connect(function()
        -- para no PRIMEIRO destes: tempo esgotado, personagem removido,
        -- ja morreu, ou o respawn ja trocou o personagem. O RESET.txt so
        -- olhava tempo e char.Parent, entao continuava zerando a vida e
        -- empurrando 1e7 depois da morte -- e era isso que emendava um
        -- reset no outro.
        if tick() - t0 > 0.15
        or not char.Parent
        or hum.Health <= 0
    ...
    -- rede de seguranca: aconteca o que acontecer, em 12s as travas caem
    -- e o hook do metatable sai. Sem isso um caminho de erro deixaria o
    -- reset morto pro resto da sessao.
    ...
    task.spawn(function()
        -- TETO DE TEMPO. O RESET.txt usa CharacterAdded:Wait() puro: se o
        -- respawn nao vier, o hook do metatable fica pendurado pra sempre e
        -- o cooldown nunca libera -- o reset morre em silencio dali pra
        -- frente. Aqui, se em 10s nada acontecer, desfaz tudo e libera.
        local novo
        local t = tick()
        repeat
            novo = lp.Character
        ...
        task.wait(0.3)                       -- deixa o respawn assentar
        ...
        if nRoot then
            -- um stud acima pro pulo registrar direito
...
local _sabcomResetBindable = Instance.new("BindableEvent")
_sabcomResetBindable.Event:Connect(function() pcall(instantReset) end)
task.spawn(function()
    for _ = 1, 12 do
        local ok = pcall(function()
            game:GetService("StarterGui"):SetCore("ResetButtonCallback", _sabcomResetBindable)
        end)
...
_G.SabcomInstantClone = instantClone -- exposed so the auto Clone-TP engine reuses the exact manual method
            ...
            if isStealing and not wasStealingForInvis then
                if not _G.invisibleStealEnabled and _G._forceInvisToggle then
                    task.spawn(function()
                        ...
                        -- of the brainrot (default 2s, tune via _G.SabcomInvisHoldTime).
                        local _h0 = os.clock()
...
-- ============================================================
--  CARPETE: DESLIGA SOZINHO
--
--  Por que trocar de item nao bastava: o Heartbeat logo acima
--  RE-EQUIPA o carpete todo frame --
--    if not c:FindFirstChild(tn) then hum:EquipTool(tb) end
--  entao o item que voce punha na mao era desfeito no frame
--  seguinte. A unica saida e derrubar setCarpetSpeed(false), que
--  desconecta esse loop.
--
--  Checamos o objeto que CHEGOU (ch), nao o "tool atual" do
--  personagem: no instante do ChildAdded os dois podem coexistir
--  e a leitura sairia ambigua.
-- ============================================================
do
...
-- Skips "Overhead" (nametag). Toggle: _G.SabcomHideAccessories(false)/(true)
-- =====================================================================
do
    local Players = game:GetService("Players")
    local accOn = true
    ...
    _G.SabcomHideAccessories = function(v)
        accOn = (v ~= false)
        if accOn then
...
local function isMyPlot_Instant(plotName)
    ...
    pcall(function()
        ...
        if Synchronizer then
            local ch = _G.Sabcom_GetPlotChannel(plotName)
            if ch then
...
function setStealMode(mode)
    ...
    -- Bridge to sabcom Clone-TP engine
    if _G.SabcomStealMode then pcall(_G.SabcomStealMode, mode) end
end
...
-- ============================================================
-- PRIORITY ALERT (sabcom Styled 3D Viewer with Sorting Queue)
-- ============================================================
...
task.spawn(function()
    ...
    -- ========================================================
    --  SCAN DE PLOT  --  metodo do scanner.txt, ponta a ponta
    --
    --  Esta e a funcao que alimenta o Steal Target. O que ela
    --  fazia antes e por que a lista vinha vazia:
    --
    --   1. `if not ch then return end`
    --      base sem canal registrado no cliente sumia inteira.
    --
    --   2. `not Players:FindFirstChild(owner.Name)`
    --      exigia o dono ONLINE. Base de quem deslogou era apagada
    --      do cache -- e roubar de base de gente offline e
    --      justamente o caso mais comum.
    --
    --   3. `_G.sProp(ch,"AnimalList")`
    --      cache do CLIENTE: vazio enquanto o pet nao carrega na
    --      cena, que e o problema que o scanner descreve no topo.
    --
    --  Agora: RequestData:InvokeServer -> AnimalList do SERVIDOR,
    --  dono resolvido por data.Owner com PlotSign de reserva, e
    --  nenhuma exigencia de o dono estar presente.
    -- ========================================================
    -- ========================================================
    --  CANAL DO PLOT  --  por Order, com UUID de reserva
    --
    --  ESTA ERA A FALHA. Todo o hub resolvia canal por plot.Name,
    --  que e o UUID do Model. O jogo passou a nomear os canais
    --  como Plot1, Plot2... entao XenSyncGet(uuid) devolvia nil
    --  pra TODAS as bases e o scan morria antes de ler qualquer
    --  coisa -- sem erro, so lista vazia.
    --
    --  Recebe o Instance, nao o nome: o Order so existe no Model.
    --  Nao cria canal ausente e nao faz chamada de rede.
    -- ========================================================
    local function _canalDoPlot(plot)
        if not plot then return nil end

        -- UUID PRIMEIRO, "PlotN" como reserva -- invertido de proposito.
        --
        -- O UUID e a chave do proprio Model: se resolve, e inequivocamente
        -- o canal DAQUELE plot. Ja "Plot"..Order e um palpite: nada
        -- confere se o canal pertence a esta base. Se o Order for 0-based
        -- ou estiver deslocado da numeracao dos canais, casa os podiums de
        -- um plot com a lista de slots de outro -- e getPetPosition devolve
        -- uma posicao REAL, plausivel, no podium errado. Como o andar sai
        -- do Y do podium, o TP voa pro lado oposto da base.
        --
        -- Tentar o inequivoco antes do palpite custa nada e elimina isso.
        local ch = _G.SabcomSyncGet and _G.SabcomSyncGet(plot.Name)
        if type(ch) == "table" then return ch end

        local ord
        pcall(function() ord = plot:GetAttribute("Order") end)
        if ord ~= nil then
            local ch2 = _G.SabcomSyncGet and _G.SabcomSyncGet("Plot" .. tostring(ord))
            if type(ch2) == "table" then return ch2 end
        end
        return nil
    end

    -- leitura na ordem pedida: CacheTable -> campo direto -> :Get
    ...
    -- os pets vem em Animals ou AnimalList, dependendo do servidor
    -- AnimalList PRIMEIRO. A spec pedia Animals antes, mas todo o resto do
    -- arquivo (2247, 4247, 9196, o _G.Sabcom_GetPlotAnimalList da linha 271) e
    -- o proprio scanner.txt leem SOMENTE AnimalList -- e e nas chaves dessa
    -- tabela que os nomes dos podiums batem. Se Animals for indexado de
    -- outro jeito (array 1..n, UUID), o slot cai noutro namespace e
    -- getPetPosition resolve um podium errado, sem erro nenhum.
    -- Animals fica como reserva, para servidor que so tenha ele.
    ...
    -- a entrada aparece em cinco formatos diferentes
    ...
    -- ============================================================
    --  UMA TENTATIVA SO, NAO UMA POR SCAN
    --
    --  Antes o `if _genFix then return end` so barrava depois de um
    --  SUCESSO -- e sucesso aqui nunca acontece: AnimalsShared no
    --  hub e _G._xenAnimShim, cujo GetGeneration e
    --
    --     function(_, index, mutation, traits)
    --         return _G._xenGen(index, mutation, traits)
    --     end
    --
    --  uma closure SEM UPVALUE NENHUM. Entao debug.getupvalue(f, 2)
    --  falha sempre, _genFix nunca vira true, e o pcall inteiro se
    --  repetia por plot, a cada 0.5s, pra sempre.
    --
    --  Agora marca como TENTADO na primeira vez, deu certo ou nao.
    -- ============================================================
    ...
    -- nome do dono: data.Owner em qualquer das formas, e PlotSign como
    -- reserva. O scanner faz exatamente assim.
    ...
    -- formatacao igual a do jogo, com fallback que TRUNCA em vez de
    -- arredondar: 1.95B vira "1.9B", nunca "2B"
    ...
    -- ========================================================
    --  FALLBACK: MODELS RENDERIZADOS
    --  So filhos DIRETOS de Plots.<UUID>. Varrer o workspace todo
    --  traria pets de evento, maquinas e personagens.
    -- ========================================================
    local _catalogo   -- nome normalizado -> Index oficial
    ...
    -- ultimo recurso pro MPS: le o "$X/s" que o proprio pet mostra
    ...
    -- o slot vem do podium mais proximo, nunca do pivot do Model:
    -- o destino do TP tem que continuar sendo o sistema de podiums
    -- posicoes dos podiums em cache: eles nao se movem, e recalcular
    -- GetPivot de todos, pra cada Model, a cada scan, era o custo que
    -- obrigava o intervalo a ficar alto
    local _podCache = {}
    local function _podiumsDe(plot)
        ...
        for _, pod in ipairs(filhos) do
            -- SO podium de verdade. O nome do slot e sempre numerico (a
            -- linha 9192 do hub ja assumia isso). Sem esta guarda, um
            -- "Decorations" virava slot: uid "<uuid>_Decorations", e o
            -- getPetPosition resolvia o pivot desse objeto -- posicao e
            -- andar arbitrarios, que e o TP indo pro nada.
        ...
        -- so guarda quando TUDO ja replicou. Com StreamingEnabled o folder
        -- aparece parcialmente, e cachear 2 de 8 podiums congelava o mapa
        -- incompleto pela sessao inteira -- os outros 6 nunca mais eram vistos.
        local numericos = 0
    ...
    local function scanSinglePlot(plot) pcall(function()
        ...
        -- dono pelo campo Owner do canal, com os fallbacks antigos.
        -- "SUA BASE"/"YOUR BASE" no PlotSign nao prova propriedade.
        local ownerName = ""
        if ch then
        ...
        local function addPet(idx, mut, traits, slot, mpsFallback, viaSync)
            ...
            -- CHAVE SO POR SLOT. Com o Index junto, as duas passagens so
            -- colidiam quando ja concordavam sobre o animal -- ou seja,
            -- exatamente quando nao importava. Se o canal dizia slot 5 =
            -- Cow e o Model renderizado ainda era o Dragon anterior, as
            -- chaves diferiam e OS DOIS entravam: duas entradas com o
            -- mesmo uid e o mesmo slot, e o TP ia parar no pet errado.
            local chave = nome .. ":" .. tostring(slot)
            if vistos[chave] then return end          -- sync tem preferencia
            vistos[chave] = true
            ...
            -- mpsFallback vem como FUNCAO, nao valor. Antes vinha ja
            -- calculado, e Lua avalia argumento antes de chamar --
            -- entao a varredura de GetDescendants dentro do
            -- _mpsDoTexto rodava pra TODO pet renderizado, de TODO
            -- plot, a cada 0.5s, mesmo quando o GetGeneration acima
            -- tinha dado certo e o valor era jogado fora.
        ...
        for _, m in ipairs(plot:GetChildren()) do
            if m:IsA("Model") then
                ...
                if idx then
                    ...
                    if slot then
                        -- passa a FUNCAO, nao o resultado: so e calculado
                        -- se o GetGeneration falhar
        ...
        -- HASH SOBRE O RESULTADO REAL, nao sobre a contagem.
        --
        -- Antes era getAnimalHash(al) .. "|r" .. #novos. Tres buracos:
        --  * sem canal, getAnimalHash(nil) = "" e sobrava so a contagem.
        --    Trocar o pet do podium 3 mantem a contagem -> o cache
        --    congelava com o pet antigo pra sempre. Pet mudando de podium
        --    2 pro 6 tambem passava batido, e o TP ia num podium vazio.
        --  * Traits nao entravam no hash, mas entram no GetGeneration ->
        --    genValue e ordenacao ficavam velhos.
        --  * ownerName nao entrava. Para a propria base o nome so resolve
        --    quando o canal chega; se a lista nao mudasse nessa janela o
        --    "" ficava gravado -- e todo filtro e `owner ~= LocalPlayer`,
        --    que "" atravessa. Resultado: auto-steal mirando na sua base.
        local partes = { ownerName }
        for _, p in ipairs(novos) do
            partes[#partes+1] = p.slot .. "|" .. p.index .. "|" .. p.mutation .. "|" .. p.traits
        end
        table.sort(partes)          -- pairs() nao tem ordem estavel
        ...
        -- diagnostico: so imprime quando o quadro muda
        local plotsN = 0
    ...
    local function setupPlotListener(plot, phase) local ch; local retries=0
        -- ====================================================
        --  ESTE ERA O PORTAO QUE MATAVA O STEAL TARGET
        --
        --  Era assim:
        --      ... _G.Sabcom_GetPlotChannel(plot.Name) ...
        --      if not ch then return end
        --
        --  Dois defeitos somados:
        --
        --   1. resolvia o canal por plot.Name (o UUID do Model).
        --      Com o jogo nomeando canais como Plot1/Plot2, isso
        --      dava nil em TODAS as bases.
        --
        --   2. sem canal, dava `return` -- entao scanSinglePlot
        --      NUNCA era chamado e o laco de 0.5s nunca comecava.
        --      Nao adiantava consertar o scan: ele nao rodava.
        --
        --  Agora resolve por Order (com UUID de reserva) e, o mais
        --  importante, NAO desiste se o canal faltar: o fallback de
        --  Models renderizados funciona sem canal nenhum.
        -- ====================================================
        -- PRIMEIRO SCAN IMEDIATO, sem esperar canal nenhum.
        -- Antes o scan so acontecia DEPOIS do laco de ate 40x0.07s (~2.8s)
        -- -- e esse tempo era pago mesmo quando os Models renderizados ja
        -- estavam na cena e davam alvo na hora.
        scanSinglePlot(plot)

        -- o canal e resolvido em paralelo; quando chegar, rescaneia na hora
        task.spawn(function()
            local c2, r2 = nil, 0
        ...
        -- ====================================================
        --  DETECCAO INSTANTANEA POR EVENTO
        --
        --  O laco de 0.5s vira apenas rede de seguranca. Quem da a
        --  resposta imediata sao estes dois eventos: pet colocado ou
        --  retirado do plot e um filho DIRETO entrando ou saindo.
        --
        --  De proposito ChildAdded e nao DescendantAdded: o segundo
        --  dispara milhares de vezes enquanto as partes de cada base
        --  carregam no inicio, e era exatamente o que travava o jogo
        --  antes (o comentario original do hub explica isso).
        --
        --  Debounce de 0.08s porque um pet aparece como varias pecas
        --  em sequencia; sem ele seriam varios scans para um evento.
        -- ====================================================
        -- debounce baixado de 0.08 pra 0.03: um pet aparece como
        -- varias pecas em sequencia e o debounce junta isso num scan
        -- so, mas 80ms era mais do que o necessario pra agrupar
        local _pend = false
        ...
        -- 0.25s em vez de 0.5s. Ficou barato o bastante pra isso: o
        -- _fixGen deixou de repetir por scan, o _mpsDoTexto so roda
        -- quando e mesmo necessario, e o portao de hash ja evita
        -- reconstruir a lista quando nada mudou. O dobro de leituras
        -- custa pouco e o alvo aparece na metade do tempo.
    ...
    if _G.AutoInvisDuringSteal then
        if isStealing and not _G.invisibleStealEnabled and _G._forceInvisToggle then
            task.spawn(function()
                ...
                -- 2s, tune via _G.SabcomInvisHoldTime).
                local _h0 = os.clock()
                local _need = tonumber(_G.SabcomInvisHoldTime) or 1.25
                while player:GetAttribute("Stealing") and (os.clock() - _h0) < _need do
                    task.wait(0.05)
...
local function fireGrapple(destino)
    ...
    -- o item e bloqueado pelo jogo enquanto Stealing esta ativo
    if LP:GetAttribute("Stealing") then return end

    -- mira propria + Activated (ver sabcomFireGrapple2)
    if _G.SabcomFireGrapple2 then pcall(_G.SabcomFireGrapple2, destino) end
end

-- so o tapete. O ciclo de grapple que existia aqui (equipar,
-- disparar sem argumento, esperar 0.22, desequipar, esperar 0.10)
-- foi removido: o hook e engatado antes pelo sabcomGrappleSubirEHook,
-- e essas esperas fixas so seguravam o TP.
...
-- =====================================================================
-- Aceita o Instance do plot OU o nome. O canal do Synchronizer passou a
-- se chamar "PlotN", onde N e o atributo Order do Model -- procurar por
-- plot.Name (o UUID) devolve nil em servidor novo, que era o que deixava
-- o Steal Target vazio. Order primeiro, UUID como reserva.
local _chCache = {}
...
local function getPlotChannel(plotRef)
    ...
    for _, n in ipairs(tentativas) do
        local ch
        pcall(function() ch = _G.SabcomSyncGet(n) end)
        if type(ch) == "table" then _chCache[chave] = ch; return ch end
    end

    -- SEM espera bloqueante. O loop antigo segurava a thread ate 3s POR
    -- CHAMADA, e isto e chamado por plot, em varios lugares -- era ele o
    -- travamento do inicio. Quem precisa do canal ja tem quem reescaneie
    -- quando ele aparecer; aqui devolver nil na hora e melhor que travar.
    return nil
end
...
-- ============================================================
--  LEITURA DE PLOT VIA RequestData   (metodo do scanner.txt)
--
--  Antes: channelGet(channel, "AnimalList") -- cache do CLIENTE.
--  Quando o pet nao esta carregado na cena, esse cache vem vazio e
--  o Steal Target aparece sem nada, sem erro nenhum.
--
--  Agora: RequestData:InvokeServer(plot.Name) pergunta ao SERVIDOR,
--  entao a lista vem completa independente do que o cliente
--  carregou. Mesma fonte que o scanner usa.
-- ============================================================
local _RequestData
...
-- InvokeServer faz round-trip. O scanner e disparo unico, mas aqui
-- scanAllPets roda em ciclo -- sem esta janela curta seriam dezenas
-- de invokes por segundo, e o servidor comeca a estrangular.
...
-- O scanner corrige GetGeneration antes de usar: sem isto ela devolve
-- valor errado fora do contexto que o jogo espera.
...
local function scanAllPets()
    ...
    for _, plot in ipairs(Plots:GetChildren()) do
        ...
        for slot, animalData in pairs(animalList) do
            ...
            -- ovo e lucky block nao sao pet: aparecem na AnimalList com
            -- geracao invalida e viravam alvo fantasma no Steal Target
            ...
            -- descarta o que nao virou numero, em vez de tratar como 0 --
            -- com 0 o pet entrava na lista e afundava na ordenacao,
            -- parecendo que existia mas sem valor nenhum
...
local function vizPath(fromPos, waypoints)
    ...
    -- Toggle off with _G.SabcomShowTPPath = false.
...
local function velMoveThrough(hrp, waypoints, speedOverride, allowJump, quickStart)
    ...
    local _runSpeed = speedOverride or Config.TpSettings.GrabbleTPSpeed or (_G.SabcomCarpetSpeed or CARPET_SPEED or 230)
    ...
    local function finish()
        ...
        task.delay(tonumber(_G.SabcomTPPathLinger) or 1.5, function()
...
function getPetPosition(plot, slot)
    ...
    -- (0,0,0) e um Vector3 TRUTHY: passava por `if pos then` em todo lugar
    -- e virava destino, mandando o TP pra origem do mundo -- uns 400 studs
    -- fora do mapa, atravessando o vazio. Da rollback na certa. A linha 345
    -- deste mesmo arquivo ja usa esta guarda.
...
function runAutoSnipe()
    ...
    if Config.TpSettings and Config.TpSettings.GrabbleTP then
        _G._isTpMoving = true
        local fn = _G.SabcomStartSideTP or doGrabbleVelocityTP
    ...
    pcall(setfpscap, 200)
    -- (Old Grabble TP branch removed - now handled by sabcom Clone-TP router above.)
    _G._isTpMoving = true
...
local function buildRemoteSell()
    ...
    local function findMyPlotRS()
        ...
        for _,plot in ipairs(plots:GetChildren()) do
            if Synchronizer_rs then local ok,ch=pcall(function() return _G.SabcomSyncGet(plot.Name) end)
    ...
    remoteSellGui=Instance.new("ScreenGui"); remoteSellGui.Name="sabcom_RemoteSell"; remoteSellGui.ResetOnSpawn=false; remoteSellGui.Parent=playerGui
...
_G.ShowStealProgressBar = function(targetName, duration)
    ...
    sg.Name = "sabcom_StealProgressBar"
...
-- ============================================================
--  CAPTURA DE KEYBIND  (compartilhada pelas caixinhas)
--
--  A versao anterior guardava o estado num `ouvindo` local de cada
--  caixa, e vazava de tres jeitos:
--
--   1. DUAS CAIXAS ARMADAS. Clicar na segunda e um clique de GUI,
--      entao chegava com gp=true na primeira, que fazia `return`
--      sem desconectar -- ficava armada. A tecla seguinte era
--      gravada nas DUAS, e o dispatcher, que retorna no primeiro
--      match, deixava uma das acoes inalcancavel pra sempre.
--
--   2. SEM CANCELAMENTO. Qualquer entrada que nao fosse teclado ou
--      M3 caia no `else return end` e mantinha a escuta viva.
--
--   3. SOBREVIVIA A PROPRIA MORTE. A caixa do Face Nearest fica em
--      mainBody, e loadTab comeca com clearBody(mainBody). Armar a
--      caixa e trocar de aba destruia o botao mas nao a conexao:
--      a proxima tecla qualquer -- um W andando -- era gravada como
--      bind, salva em disco, silenciosamente.
--
--  Agora existe UMA escuta global por vez, ela e cancelada ao
--  armar outra, ao destruir a caixa e por tempo.
-- ============================================================
local _capAtiva = nil
...
function capturarBind(kb, bindName, rotulo)
    ...
    -- caixa destruida (troca de aba) cancela a escuta
    ...
    -- teto de tempo: escuta abandonada nao fica esperando pra sempre
...
function openAnim(f) if not f then return end; local us=f:FindFirstChild("sabcomScale") or Instance.new("UIScale"); us.Name="sabcomScale"; us.Parent=f
...
function makeHeader(f,t,isMain)
    local bar=Instance.new("Frame"); bar.Name="sabcomBar"; bar.Size=UDim2.new(1,0,0,2)
    bar.BackgroundColor3=(sabcom and sabcom.GRAD_A) or Theme.Accent; bar.BorderSizePixel=0; bar.ZIndex=5; bar.Parent=f
    local g=Instance.new("UIGradient"); g.Rotation=0
    g.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,(sabcom and sabcom.GRAD_A) or Theme.Accent),
                               ColorSequenceKeypoint.new(1,(sabcom and sabcom.GRAD_B) or Theme.AccentLight)}); g.Parent=bar
    ...
    local function marca(txt)
        ...
        return (txt:gsub("sabcom HUB PRIVAT",(sabcom and sabcom.NOME) or "sabcom"))
    end
...
function makeSyncStateRow(parent,text,toggleName,callback,bindName)
    ...
    if bindName then
        local kb=Instance.new("TextButton"); kb.Name="sabcomBind"; kb.Size=UDim2.new(0,56,0,22)
...
function makeSyncMainToggle(parent,text,toggleName,callback,bindName)
    ...
    -- caixinha de keybind: entra entre o label e o switch. O switch ocupa
    -- 38px em (1,-44), entao a caixa fica em (1,-104) com 56 e sobram 6 de
    -- folga dos dois lados.
    if bindName then
        local kb=Instance.new("TextButton"); kb.Name="sabcomBind"; kb.Size=UDim2.new(0,56,0,21)
...
-- CREATE PANELS
-- O titulo estava vazio, entao o cabecalho da janela principal nao
-- mostrava nada. "sabcom Hub" tambem vira a chave de posicao/tamanho
-- salvos (Config.positions[t] / Config.sizes[t]) -- a antiga era ""
-- e nao batia com o applySavedPosition("sabcom HUB PRIVAT") logo abaixo,
-- entao a janela nunca restaurava a posicao mesmo.
main,mainBody=makeMainPanel("sabcom Hub",UDim2.new(0,375,0,480),UDim2.new(0.5,-187,0.5,-255))
if Config.AutoCloseOnExec then main.Visible = false end
panels["Invisible Steal Panel"],panels["InvisStealBody"]=makeQuickPanel("sabcom HUB PRIVAT\nInvisible Steal",UDim2.new(0,230,0,375),UDim2.new(0,80,0.5,-220))
panels["InvisStealBody"].ScrollBarThickness = 0
panels["InvisStealBody"].ScrollingEnabled = false
panels["Admin Command Panel"],panels["AdminBody"]=makeQuickPanel("sabcom HUB PRIVAT\nAdmin Command Panel",UDim2.new(0,225,0,240),UDim2.new(0.5,85,1,-340))
panels["Command Cooldowns"],panels["CooldownBody"]=makeQuickPanel("sabcom HUB PRIVAT\nCommand Cooldowns",UDim2.new(0,210,0,315),UDim2.new(0.5,245,1,-390))
panels["Actions"],panels["ActionsBody"]=makeQuickPanel("sabcom HUB PRIVAT\nActions",UDim2.new(0,230,0,340),UDim2.new(0.5,505,1,-415))
panels["Steal Panel"],panels["StealBody"]=makeQuickPanel("sabcom HUB PRIVAT\nSteal Panel",UDim2.new(0,235,0,300),UDim2.new(1,-300,1,-385))
panels["Steal Target"],panels["TargetBody"]=makeQuickPanel("sabcom HUB PRIVAT\nSteal Target",UDim2.new(0,320,0,380),UDim2.new(1,-330,0,85))
actionSettingsPanel,actionSettingsBody=makeQuickPanel("sabcom HUB PRIVAT\nAction Settings",UDim2.new(0,230,0,370),UDim2.new(0.5,745,1,-440))
actionSettingsPanel.Visible=false
tpSpeedSettingsPanel,tpSpeedSettingsBody=makeQuickPanel("sabcom HUB PRIVAT\nTP & Clone Settings",UDim2.new(0,235,0,325),UDim2.new(0.5,745,1,-440))
tpSpeedSettingsPanel.Visible=false
-- "sabcom Hub" aqui tambem: e a mesma chave usada em makeMainPanel.
-- Antes era "sabcom HUB PRIVAT" enquanto o painel salvava sob "" --
-- chaves diferentes, entao a posicao nunca voltava.
for _,pair in ipairs({{"sabcom Hub",main},{"sabcom HUB PRIVAT\nInvisible Steal",panels["Invisible Steal Panel"]},
    {"sabcom HUB PRIVAT\nAdmin Command Panel",panels["Admin Command Panel"]},{"sabcom HUB PRIVAT\nCommand Cooldowns",panels["Command Cooldowns"]},
    {"sabcom HUB PRIVAT\nActions",panels["Actions"]},{"sabcom HUB PRIVAT\nSteal Panel",panels["Steal Panel"]},{"sabcom HUB PRIVAT\nSteal Target",panels["Steal Target"]},
    {"sabcom HUB PRIVAT\nAction Settings",actionSettingsPanel},{"sabcom HUB PRIVAT\nTP & Clone Settings",tpSpeedSettingsPanel}}) do applySavedPosition(pair[1],pair[2]) end
...
for name, panel in pairs(panels) do
    if not string.match(name, "Body$") then
        -- _G._sabcomPanelVis survives re-executes; override Config if it has a value
        local fromG = _G._sabcomPanelVis[name]
        local targetVis
        if fromG ~= nil then
            targetVis = fromG
...
function rebuildTpSpeedSettings()
    ...
    makeMainSliderWithInput(tpSpeedSettingsBody, "Grabble TP Speed", 50, 600, Config.TpSettings.GrabbleTPSpeed or 230, function(v) Config.TpSettings.GrabbleTPSpeed=v; saveConfig(); if _G.SabcomSetCarpetSpeed then pcall(_G.SabcomSetCarpetSpeed, v) end end)
...
function spamBaseOwner()
    ...
    if ok and Synchronizer then
        local ch = _G.Sabcom_GetPlotChannel(nearestPlot.Name)
        if ch then
            local owner = _G.sProp(ch, "Owner")
            if owner then
                if (typeof(owner) == "Instance") and owner:IsA("Player") then
...
makeSyncStateRow(panels["StealBody"],"Auto Steal:","Auto Steal",function(on)
    ...
    -- sabcom Clone-TP engine owns the auto-steal loop unconditionally.
...
makeSyncStateRow(panels["StealBody"],"Auto Buy:","Auto Buy",function(on)
    ...
    else warn("[sabcom] Auto Buy not ready yet -- try again in a sec") end
end)
        ...
        overlay.MouseButton1Click:Connect(function()
            ...
            if manuallySelectedUID then
                task.spawn(function()
                    ...
                    if pr then
                        if _G.Sabcom_ExecuteManualSteal then
                            pcall(_G.Sabcom_ExecuteManualSteal, pr)
                        end
                    end
                end)
            end
...
LazyInit("Admin Panel UI", function() -- ADMIN PANEL UI SCOPE
    ...
    function createAPRow(plr)
        ...
        if _G.SabcomIsBadBoy and _G.SabcomIsBadBoy(plr) then
            nameLabel.Text = plr.DisplayName .. '  <font color="#ff5555">[BAD BOY]</font>'
        elseif _G.SabcomIsGoodBoy and _G.SabcomIsGoodBoy(plr) then
...
function loadTab(tabName)
    ...
    if tabName=="Keybinds" then
        ...
        -- "Steal Nearest" e "Face Nearest" entraram na tabela Keybinds mas
        -- faltavam aqui: ficavam sem linha, sem exibicao e sem o botao X
        -- de limpar -- unico jeito de desfazer um bind errado.
    ...
    elseif tabName=="Auto TP" then
        ...
        makeMainToggle(mainBody,"Grabble TP",Config.TpSettings.GrabbleTP,function(on)
            ...
            saveConfig()
            -- Grabble TP IS the sabcom Clone-TP engine. Sync auto-steal loop state.
            if _G.SabcomAutoSteal then
                pcall(_G.SabcomAutoSteal, on and (Config.AutoStealEnabled or false))
            end
        end)
    ...
    elseif tabName=="UI" then
        -- sabcom EXTRAS: guardado porque loadTab pode rodar na inicializacao,
        -- antes do modulo do fim do arquivo existir
        if _G.sabcomExtras then
            makeSyncMainToggle(mainBody,"Face Away","Face Away",function(on) _G.sabcomExtras.setFaceAway(on) end)
            makeSyncMainToggle(mainBody,"Face Nearest Player","Face Nearest",function(on) _G.sabcomExtras.setFaceNearest(on) end,"Face Nearest")
        end
    ...
    elseif tabName=="Misc" then
        if _G.sabcomExtras then
            makeSyncMainToggle(mainBody,"Anti Colisao (atravessar players)","Anti Colisao",function(on) _G.sabcomExtras.setAntiColisao(on) end)
            makeSyncMainToggle(mainBody,"Anti Flasher (remove acessorios)","Anti Flasher",function(on) _G.sabcomExtras.setAntiFlasher(on) end)
            makeSyncMainToggle(mainBody,"Ps On Steal","Ps On Steal",function(on) _G.sabcomExtras.setPs(on) end)
        end
...
RunService.RenderStepped:Connect(function() frames=frames+1; local now=tick(); if now-lastT>=1 then local fps=frames; _G.currentFPS=fps; frames=0; lastT=now; local ping=0; pcall(function() ping=math.floor(LocalPlayer:GetNetworkPing() * 1000) end); if fpsText and fpsText.Parent then pcall(function() fpsText.Text="FPS: "..fps.."\nPING: "..ping.."ms" end) end
    -- mesma leitura alimenta a barra do topo, numa linha so
    local st = _G.__sabcomStats
...
UIS.InputBegan:Connect(function(input,gp) if gp then return end
    ...
    local actions={
        ...
        -- Route Manual TP through the SAME sabcom engine that TP-on-load uses (which
        ...
        -- CapsLock. Roda o MESMO ciclo do TP-on-load:
        --   sobe com o tapete -> hook reto pra baixo -> TP
        ["Manual TP"]=function()
            if _G.SabcomCicloTP then task.spawn(_G.SabcomCicloTP)
            elseif _G.Sabcom_ExecuteManualTP then _G.Sabcom_ExecuteManualTP()
            elseif _G.SabcomStartSideTP then task.spawn(_G.SabcomStartSideTP)
            else task.spawn(runAutoSnipe) end
        end,
        ["Steal Nearest"]=function()
            -- Priority e o modo principal. O bind so empresta o
            -- Nearest; apertar de novo devolve pra Priority.
        ...
        -- Face Away: liga vira pro nearest, desliga volta pro base owner.
        -- Mesma logica do Steal Nearest -- base owner e o modo principal.
        ["Face Nearest"]=function()
            if _G.sabcomExtras then
                _G.sabcomExtras.setFaceNearest(not Config.FaceAwayNearest)
            end
        end,
    ...
    _G.__sabcomActions = actions
    for actionName,keyBind in pairs(Keybinds) do if keyBind==kn and actions[actionName] then actions[actionName](); return end end
end)

-- Os botoes laterais do mouse NAO existem no Roblox: Enum.UserInputType
-- tem MouseButton1, 2 e 3 e para por ai. Referenciar MouseButton4/5 nao
-- e um bind que nunca dispara -- e erro em tempo de execucao, e derrubava
-- o hub aqui. O caminho pra usar o botao lateral e remapear ele pra uma
-- tecla (F13, por exemplo) no software do proprio mouse e bindar a tecla.
...
if Config.TpSettings.TpOnLoad then task.spawn(function()
    ...
    -- RACE FIX: _G.SabcomStartSideTP is assigned ~1800 lines below this block. If
    ...
    local _t0 = os.clock()
    while not _G.SabcomStartSideTP and os.clock() - _t0 < 10 do
        RunService.Heartbeat:Wait()
    end
    ...
    -- ========================================================
    --  ESPERA O ALVO **ANTES** DE PREPARAR
    --
    --  Esta era a razao de o hook engatar e o TP nao sair.
    --
    --  A ordem antiga era: preparo -> TP. Só que no instante do
    --  load o cache de pets ainda esta vazio, entao o TP entrava,
    --  nao achava alvo, e ficava ate 6 segundos procurando --
    --  tempo em que o puxao do grapple ja tinha acabado. Quando
    --  enfim achava, partia sem o estado que o hook cria, que e
    --  justamente o que faz o TP funcionar.
    --
    --  Agora: espera aparecer pet de base, e SO ENTAO sobe,
    --  engata e teleporta -- tudo em sequencia, sem o hook
    --  esfriando no meio.
    -- ========================================================
    do
        local _tp0 = os.clock()
    ...
    -- ========================================================
    --  PREPARO: SOBE COM O TAPETE E ENGATA O GRAPPLE NO CHAO
    --
    --  A subida existe porque o item exige de 10 a 50 studs ate o
    --  alvo. Parado no chao, o ponto sob o pe esta a ~0 -- fora da
    --  faixa, e o disparo seria descartado.
    --
    --  A mira e por camera e nao depende do cursor, entao isto
    --  funciona no TP-on-load, quando o mouse pode estar em
    --  qualquer lugar da tela.
    -- ========================================================
    -- o ciclo inteiro numa chamada so, o mesmo que o CapsLock usa.
    -- No load damos 25s de teto pra esperar o alvo: acabamos de
    -- entrar e o scanner ainda esta populando. No CapsLock o teto e
    -- 4s, porque ali voce quer resposta imediata.
    if _G.__LMARK then _G.__LMARK("TP-on-load: iniciando ciclo") end
    if _G.SabcomCicloTP then
        _G.SabcomCicloTP(25)
    elseif _G.SabcomStartSideTP then
...
task.spawn(function()
    ...
    pcall(function()
        ...
        for _, plot in ipairs(Plots:GetChildren()) do
            task.spawn(function()
                local ch = _G.Sabcom_GetPlotChannel and _G.Sabcom_GetPlotChannel(plot.Name)
                if ch then
    ...
    pcall(function()
        local result = _G.SabcomSyncAll and _G.SabcomSyncAll()
        if result then
    ...
    print("[sabcom] TP Preload complete")
end)
...
-- ============================================================
-- sabcom AUTO-STEAL + CLONE-TP ENGINE (integrated) -- xentp.lua 1:1 port
...
do
    ...
    local function loadNet()
        ...
        if not ok or type(mod) ~= "table" then
            warn("[sabcom AutoSteal] loadNet FAILED - _G.Net not available. ok=" .. tostring(ok) .. " type=" .. type(mod))
            return false
        end
        NetModule = mod
        return true
    ...
    local function fireGrapple(destino)
        ...
        if not tool then
            ...
            if t2 and hum then
                pcall(function() hum:EquipTool(t2) end)
                task.wait(0.05)                    -- deixa o equip registrar
            end
            tool = char:FindFirstChild("Grapple Hook")
        end
        if not tool then return end

        -- o script do item comeca com
        --   if Parent:GetAttribute("Stealing") then return end
        -- entao com o pet na mao o grapple esta desligado por regra do
        -- jogo, nao por falha nossa
        if LP:GetAttribute("Stealing") then return end

        -- Mira propria + Activated. O sabcomFireGrapple2 escreve
        -- PlayerMouse.Hit/Target no ponto do CHAO na direcao do
        -- destino e aciona o item no mesmo frame -- entao a mira
        -- independe de onde o mouse esta, que e o que o TP-on-load
        -- exige. Ver o comentario longo la na definicao.
        if not (_G.SabcomFireGrapple2 and _G.SabcomFireGrapple2(destino)) then
            warn("[sabcom] fireGrapple: sem ponto de chao valido entre 10 e 50 studs na direcao do alvo")
        end
    end
    _G.SabcomFireGrapple = fireGrapple

    -- ===== Speed config (live-settable) =====
    local sabcomSpeed = { CARPET = 400, INBASE = 250 }
    _G.SabcomSetCarpetSpeed = function(v) v = tonumber(v); if v and v > 0 then sabcomSpeed.CARPET = v end end
    _G.SabcomSetInbaseSpeed = function(v) v = tonumber(v); if v and v > 0 then sabcomSpeed.INBASE = v end end
    _G.SabcomGetCarpetSpeed = function() return sabcomSpeed.CARPET end
    if Config and Config.TpSettings then
        if tonumber(Config.TpSettings.GrabbleTPSpeed) then sabcomSpeed.CARPET = tonumber(Config.TpSettings.GrabbleTPSpeed) end
    end
...
-- ============================================================
--  carpetEngage  --  SO O TAPETE, sem ciclo de grapple
--
--  Antes esta funcao fazia o ritual inteiro do grapple ANTES de
--  chegar no tapete:
--    * ate 3s esperando a Grapple Hook replicar
--    * equipa, ate 0.5s esperando o equip registrar
--    * dispara (sem argumento -> mira horizontal, a diagonal)
--    * task.wait(0.08), desequipa tudo, task.wait(0.08)
--    * so entao o tapete
--
--  Era isso que dava a impressao de "esperar o grapple acabar
--  pra dar o TP" -- somando, quase meio segundo de espera fixa,
--  e ate 3.5s no pior caso.
--
--  O hook agora e engatado pelo sabcomGrappleSubirEHook, antes do
--  TP comecar. Aqui so falta o tapete, entao a funcao vai direto
--  nele e retorna no mesmo frame quando ja esta na mao.
-- ============================================================
-- ============================================================
--  carpetEngage  --  NAO BLOQUEIA
--
--  Esta funcao roda no comeco do doVelocityTP. Enquanto ela
--  esperava o tapete registrar (ate 0.4s), o personagem ficava
--  parado no ar -- tempo suficiente pro puxao do grapple trazer
--  ele de volta pro chao. O TP so comecava depois, ja embaixo.
--
--  Era isso o "dispara o grapple, espera ser puxado, e so entao
--  da o TP".
--
--  Agora ela pede o equip e retorna NO MESMO FRAME. O tapete
--  chega em background; o movimento comeca imediatamente, ainda
--  na altura em que o hook foi disparado.
-- ============================================================
local function carpetEngage()
        local char = LP.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        ...
        -- ja esta com o tapete? sai na hora
        for _, n in ipairs(CARPET_NAMES) do
        ...
        -- se ainda nao registrou, insiste EM BACKGROUND e devolve ja
    ...
    -- ===== Plot/Channel helpers =====
    -- mesma correcao da copia de cima: canal por Order, UUID de reserva
    local function getPlotChannel(plotRef)
        if not Synchronizer then return nil end
        ...
        -- UUID primeiro (chave inequivoca do proprio Model), "PlotN" depois.
        -- Ver comentario longo em _canalDoPlot: Order-first casa podium de
        -- um plot com slot de outro se a numeracao estiver deslocada.
        local tentativas = {}
        if typeof(plot) == "Instance" then
            tentativas[#tentativas+1] = plot.Name
            local ord; pcall(function() ord = plot:GetAttribute("Order") end)
        ...
        return nil   -- sem espera bloqueante, pelo mesmo motivo da copia acima
    end
    local function channelGet(channel, key)
    ...
    -- ===== Scanner =====
    -- ========================================================
    --  RequestData (metodo do scanner.txt)
    --  Redefinido aqui porque este escopo nao alcanca os helpers
    --  do topo do arquivo.
    -- ========================================================
    -- SEM REDE. Aqui havia RequestData:InvokeServer(plot.Name), chamado
    -- por plot dentro de scanAllPets -- que roda num Heartbeat com porta
    -- de 0.1s. O marcador de tempo era gravado ANTES da chamada render,
    -- e o cache so DEPOIS, entao nao havia trava de chamada em voo: com
    -- 8 bases e ~60ms de ida e volta, uma varredura leva ~500ms e outras
    -- cinco comecavam por cima -- dezenas de invokes simultaneos, que e
    -- exatamente o que faz o servidor estrangular as respostas.
    --
    -- Agora le o canal do Synchronizer, igual ao scanSinglePlot, e
    -- devolve no mesmo formato { AnimalList = ... } que o resto espera.
    local function _readPlot(plot)
        ...
        pcall(function()
            ...
            if type(ct) == "table" then
                -- AnimalList primeiro: e nas chaves dela que os nomes dos
                -- podiums batem (ver comentario em _listaDePets)
    ...
    -- ========================================================
    --  ALVO DIRETO PELO UID DO PAINEL
    --
    --  O painel (scanSinglePlot -> SharedState.AllAnimalsCache) e o
    --  motor (scanAllPets, aqui) montam DUAS listas independentes.
    --  O unico elo e manuallySelectedUID = plot.."_"..slot.
    --
    --  Quando o uid clicado nao existe na lista do motor, o codigo
    --  caia em `_pickByMode(allPets) or allPets[1]` -- em silencio,
    --  sem erro: voce clicava num pet e o TP ia noutro.
    --
    --  Isso ficou comum depois da troca do scan: o painel enxerga
    --  pets do fallback renderizado e de bases sem canal, que o
    --  motor nao lista.
    --
    --  Aqui a entrada e reconstruida do proprio uid. A posicao sai
    --  do getPetPosition de sempre (podium), nunca do pivot do
    --  Model -- entao andar e slot continuam vindo do sistema antigo.
    -- ========================================================
    local function _petPorUID(uid)
        if type(uid) ~= "string" then return nil end
        -- o UUID do plot nao tem "_", entao o ultimo separa slot
        ...
        -- nome/valor vem do que o painel ja mostrou, so pra exibicao
    ...
    local function scanAllPets()
        ...
        _corrigirGen()
        -- ====================================================
        --  FONTE UNICA: a mesma lista que o painel mostra
        --
        --  O motor tinha scan proprio. Depois que _readPlot passou a
        --  ler canal (e a desistir quando o canal nao resolve), esse
        --  scan devolvia quase nada de base -- sobravam so os pets da
        --  ESTEIRA, que vem de RenderedMovingAnimals e nao precisam de
        --  canal. Com a lista assim, allPets[1] era um pet da esteira,
        --  e era pra la que o TP ia.
        --
        --  Agora o motor consome SharedState.AllAnimalsCache, montado
        --  pelo scanSinglePlot. Uma lista so, entao painel e TP nao
        --  tem como divergir -- que era a raiz de "clico num pet e ele
        --  vai noutro".
        --
        --  A POSICAO continua saindo do getPetPosition (podium) a
        --  partir de plot+slot. O cache nao carrega posicao, e e assim
        --  que tem que ser: andar e destino seguem o sistema de
        --  podiums, nunca o pivot do Model renderizado.
        -- ====================================================
        local Plots = workspace:FindFirstChild("Plots")
        if not Plots then return pets end
    ...
    local function vizPath(fromPos, waypoints)
        ...
        -- lines. Toggle off with _G.SabcomShowTPPath = false. Helpers are inlined so
    ...
    local function velMoveThrough(hrp, waypoints, speedOverride, allowJump, quickStart)
        if not hrp or not hrp.Parent or #waypoints == 0 then return end
        local _runSpeed = speedOverride or sabcomSpeed.CARPET
        vizPath(hrp.Position, waypoints)
        local _myVizGen = _G.__vizGen
        local wpIdx = 1
        local done = false
    ...
    local function doClone()
        ...
        if not cloner then
            warn("[sabcom] doClone: Quantum Cloner not found in Backpack or Character")
            return false
        end
        ...
        warn("[sabcom] doClone: manual clone method (_G.SabcomInstantClone) unavailable")
        return false
    end
    ...
    local function goToBrainrot(petPos)
        ...
        if not _isF2FromF1Case then
            do
                ...
                _plat.Name = "sabcomTempPlatform"; _plat.Size = Vector3.new(8, 1, 8)
    ...
    local function executeStealAsync(prompt)
        ...
        _G.Sabcom_StealStatus.duration = tonumber(_G.SabcomStealHoldDuration) or STEAL_HOLD_DURATION
        ...
        task.spawn(function()
            ...
            -- Live-tunable via _G.SabcomStealHoldDuration: set it to e.g. 0.1 to steal
            ...
            local _holdDur = tonumber(_G.SabcomStealHoldDuration) or STEAL_HOLD_DURATION
    ...
    _G.SabcomAutoSteal = function(on) AUTO_STEAL = on ~= false end
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if AUTO_STEAL and _started and not LP:GetAttribute("Stealing")
            ...
            if hrp then
                ...
                if ok and pets then
                    local best
                    -- Manual selection from sabcomHub
                    if manuallySelectedUID then
                        ...
                        -- mesmo motivo do doVelocityTP: honrar o clique do
                        -- painel mesmo quando o motor nao listou o pet
    ...
    local function doVelocityTP()
        ...
        -- scanAllPets sem pcall deixava isTeleporting travado em true se
        -- ela lancasse (o table.sort com comparador nao-estrito consegue),
        -- e a guarda de 30s la em cima matava TODO TP por meio minuto.
        local _okScan, allPets = pcall(scanAllPets)
        if not _okScan or type(allPets) ~= "table" then
            isTeleporting = false; return
        end

        -- CONTA SO PET DE BASE. A espera existe pra dar tempo dos plots
        -- replicarem, mas testava #allPets -- e pet de esteira nasce na
        -- hora, sem canal. Bastava UM na correia pra guarda passar de
        -- primeira com a lista 100% esteira, e era exatamente esse o TP
        -- errado do primeiro teleporte depois de entrar.
        ...
        -- Respect sabcomHub manual selection, then fall back to priority/mode
        local pet
        if manuallySelectedUID then
            for _, p in ipairs(allPets) do
                if p.plot and p.slot and (p.plot .. "_" .. tostring(p.slot)) == manuallySelectedUID then pet = p; break end
            end
            -- nao esta na lista do motor: reconstroi do uid em vez de
            -- trocar de alvo em silencio
            if not pet then pet = _petPorUID(manuallySelectedUID) end
            -- ESCOLHA MANUAL E ABSOLUTA. Antes caia no _pickByMode logo
            -- abaixo, que NAO filtra esteira -- por isso o TP saia voando
            -- pro RenderedMovingAnimals. Se o alvo clicado nao da pra
            -- resolver, melhor nao teleportar do que ir pro lugar errado.
            if not pet then isTeleporting = false; return end
        end
        if not pet then
            -- sem escolha manual: esteira fora da disputa. O caminho de
            -- auto-steal ja filtrava assim; so o doVelocityTP nao filtrava.
            local semEsteira = {}
            for _, p in ipairs(allPets) do
        ...
        -- DISPARO ANTECIPADO REMOVIDO.
        --
        -- Aqui o grapple era disparado logo que o alvo resolvia -- ou
        -- seja, ANTES da subida. Era esse o "esta usando grappler
        -- antes de subir": o preparo levantava o personagem, mas este
        -- disparo ja tinha acontecido, no chao, sem altura nenhuma, e
        -- portanto fora da faixa de 10-50 studs que o item exige.
        --
        -- Quem engata o hook agora e o sabcomGrappleSubirEHook, na ordem
        -- certa: tapete -> sobe 20 -> grapple -> chao -> TP.
        ...
        local function livePos()
            ...
            while os.clock() - _t0 < 8 do
                ...
                -- TRAVAMENTO: sem isto, empurrava contra a parede pelos 8s
                -- inteiros. O velMoveThrough ja desiste em 18 quadros parado.
                if mag > _ultDist - 0.05 then _parado = _parado + 1 else _parado = 0 end
                _ultDist = mag
                if _parado >= 25 then break end
                equipCarpet()
                -- LIMITE VERTICAL, igual ao MAX_CLIMB=60 do velMoveThrough.
                -- Subida em velocidade cheia e a anomalia que o servidor mais
                -- detecta -- e este era o unico caminho de movimento do
                -- arquivo sem nenhuma trava.
    ...
    _G.SabcomStartSideTP = doVelocityTP
    if _G.__LMARK then _G.__LMARK("engine ready (TP fn defined)") end

    _G.Sabcom_ExecuteManualTP = function()
...
-- #                                                          #
-- #   sabcom EXTRAS                                          #
-- #   Face Away  +  Ps On Steal  +  Anti Flasher             #
-- #                                                          #
-- #   Tres scripts que rodavam soltos, agora dentro do hub.   #
-- #   Ficam no FIM do arquivo de proposito: aqui Config,      #
-- #   saveConfig, regToggle e setToggle ja existem, entao     #
-- #   nada precisa de espera nem de _G intermediario.         #
-- #                                                          #
-- ############################################################
do
    local Plrs      = game:GetService("Players")
    ...
    -- placeId do jogo, do ps_button original. Fica em Config so pra poder
    -- ser trocado sem editar o hub.
    ...
    -- ========================================================
    --  1. ANTI FLASHER  (remove Accessory de personagens/NPCs)
    --
    --  O original era Script de ServerScriptService e usava
    --  Players.PlayerAdded, que num executor NUNCA dispara pra
    --  quem ja esta no servidor. Aqui varremos os presentes uma
    --  vez e deixamos os eventos cuidarem do resto.
    --
    --  O loop de varredura 2x/s do original ficou de fora: em
    --  mapa de ~5000 instancias era engasgo periodico sem ganho,
    --  porque DescendantAdded ja pega tudo que nasce depois.
    -- ========================================================
    local afConns = {}
    ...
    local function antiFlasherDesligar()
        ...
        table.clear(afConns)
        -- acessorios ja destruidos nao voltam ate o respawn: e client-side
    end

    -- ========================================================
    --  2. PS ON STEAL  (redireciona pro servidor privado)
    -- ========================================================
                ...
                if Config.PsOnSteal and pg then
                    for _, g in ipairs(pg:GetDescendants()) do
                        ...
                        if txt and string.find(txt, "You stole") then
                            -- Caixa PREENCHIDA -> vai pro servidor privado.
                            -- Caixa VAZIA      -> so sai do servidor.
                            --
                            -- Sem codigo padrao de proposito: um linkCode
                            -- chumbado manda voce pro servidor privado de
                            -- outra pessoa, com o pet na mao, sem nenhum
                            -- aviso de que faltava configurar.
                            --
                            -- placeId fixo como no ps_button original: o
                            -- lugar do jogo nao muda.
                            ...
                            task.delay(0.2, function()
                                if code and code ~= "" then
                                    ...
                                    -- mesma saida que o hub ja usava no kick
    ...
    -- ========================================================
    --  BOTAO FLUTUANTE DO PS
    --
    --  Volta do ps_button original, agora na paleta do hub. Nao e
    --  um segundo estado: o clique passa pelo mesmo setPs() do
    --  toggle da Misc, e o desenho reage ao onToggleChanged. Mexer
    --  em um repinta o outro, entao os dois nunca discordam.
    -- ========================================================
    ...
    local function construirBotaoPs()
        ...
        psBtn.Name = "sabcomPsButton"
        psBtn.Size = UDim2.new(0, 132, 0, 36)
        psBtn.Position = UDim2.new(0.4, -70, 0, 30)   -- posicao do original
        psBtn.BackgroundColor3 = Theme.Panel
        psBtn.BackgroundTransparency = 0.04
        psBtn.BorderSizePixel = 0
        psBtn.ClipsDescendants = true                 -- a barra respeita o canto
        psBtn.Parent = gui
        corner(psBtn, 8)
        psStroke = stroke(psBtn, Theme.Stroke, 1, 0.2)

        -- a mesma barra de acento dos paineis: e o que faz o botao
        -- pertencer ao conjunto em vez de parecer colado
        local bar = Instance.new("Frame")
        bar.Size = UDim2.new(1, 0, 0, 2)
        bar.BackgroundColor3 = (sabcom and sabcom.GRAD_A) or Theme.Accent
        bar.BorderSizePixel = 0
        bar.ZIndex = 3
        bar.Parent = psBtn
        local g = Instance.new("UIGradient")
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, (sabcom and sabcom.GRAD_A) or Theme.Accent),
            ColorSequenceKeypoint.new(1, (sabcom and sabcom.GRAD_B) or Theme.AccentLight),
        })
        g.Parent = bar
        ...
        -- arrastar, com guarda: sem o teste de distancia, soltar o
        -- mouse depois de mover contaria como clique e o toggle
        -- inverteria sozinho toda vez que voce reposicionasse
        ...
        UIS.InputEnded:Connect(function(i)
            ...
            or i.UserInputType == Enum.UserInputType.Touch then
                if arrastando and moveu then
                    pcall(function() rememberPosition("sabcom Ps Button", psBtn) end)
                end
                arrastando = false
            end
        end)
        ...
        click.MouseButton1Click:Connect(function()
            if moveu then moveu = false; return end     -- foi arrasto, nao clique
            if _G.sabcomExtras then _G.sabcomExtras.setPs(not Config.PsOnSteal) end
        end)

        pcall(function() applySavedPosition("sabcom Ps Button", psBtn) end)
    ...
    -- ========================================================
    --  3. FACE AWAY
    --  Vira as costas pro alvo. So o Y muda, entao andar segue
    --  normal. AutoRotate precisa sair, senao o Humanoid gira
    --  de volta sozinho no frame seguinte.
    -- ========================================================
    ...
    local function donoDaBase()
        ...
        if type(fn) == "function" then
            ...
            if id then
                ...
                -- voce nunca e alvo: virar as costas pra si da vetor zero
    ...
    local function alvoAtual()
        ...
        -- BASE OWNER TRAVADO: quem era dono no instante do roubo continua
        -- sendo o alvo ate o pet sair da mao. Sem travar, ao sair do plot
        -- o "dono atual" vira outro e voce para de virar as costas pra
        -- vitima justamente na hora que importa.
        if fa.roubando then
    ...
    local function marcaGarantir()
        ...
        mkFolder = Instance.new("Folder"); mkFolder.Name = "sabcomFaceAway"; mkFolder.Parent = cam
    ...
    local function virarDeCostas(pos)
        ...
        local dir = pos - meu
        dir = Vector3.new(dir.X, 0, dir.Z)          -- altura nao conta
        if dir.Magnitude < 0.05 then return false end
        dir = dir.Unit
        hrp.CFrame = CFrame.lookAt(meu, meu - dir * 10)   -- olhar pro lado oposto
        return true
    end
    ...
    --  PAINEL DO FACE AWAY
    --
    --  Nasce na primeira vez que o toggle liga, nao antes: se
    --  fosse criado junto com o hub, ficaria ocupando memoria e
    --  posicao salva de quem nunca usa o recurso.
    --
    --  Usa makeQuickPanel pra herdar tema, borda, cabecalho,
    --  arrastar e persistencia de posicao -- a mesma base dos
    --  outros paineis, entao nasce ja combinando.
    -- ========================================================
    local painel, corpo, pAvatar, pNome, pModo, pDist, pBarra
    local linhas = {}

    -- ---------- selecao de alvo (radio) ----------
    ...
    local function escolher(kind, playerName)
        ...
        -- mantido em sincronia porque o toggle e o keybind "Face Nearest"
        -- leem esse booleano
        Config.FaceAwayNearest = (kind == "nearest")
        pcall(setToggle, "Face Nearest", Config.FaceAwayNearest)
        fa.travado   = nil          -- trocou de alvo: destrava
        fa.alvoCache = nil
        marcaLimpar()
        pintarSelecao()
        pcall(saveConfig)
    ...
    local function novaLinha(o)
        ...
        if o.foto then
            ...
            -- rbxthumb resolve sem yield. GetUserThumbnailAsync travaria a
            -- montagem da lista com o servidor cheio.
    ...
    local function reconstruirLista()
        ...
        -- ordem alfabetica pra lista nao dancar quando alguem entra ou sai
        local outros = {}
        for _, p in ipairs(Plrs:GetPlayers()) do
    ...
    local function construirPainel()
        ...
        painel, corpo = makeQuickPanel("sabcom\nPanel Face Away",
        ...
        -- faixa de estado: verde enquanto realmente esta virando as
        -- costas, cinza quando so esta armado esperando alvo
        ...
        -- a lista acompanha quem entra e sai do servidor
        Plrs.PlayerAdded:Connect(function() task.defer(reconstruirLista) end)
        Plrs.PlayerRemoving:Connect(function(p)
            -- se o alvo escolhido saiu, volta pro base owner em vez de
            -- ficar apontando pra um player que nao existe mais
        ...
        pcall(function() applySavedPosition("sabcom\nPanel Face Away", painel) end)
    end
    ...
    local function pintarPainel(alvo, d)
        ...
        local agora = os.clock()
        if agora - ultimoPaint < 0.12 then return end   -- 8x/s basta pra olho
        ultimoPaint = agora

        -- [locked] avisa que o dono foi travado no instante do roubo e nao
        -- vai mudar mesmo que voce saia do plot dele
        local m = Config.FaceAwayModo or "baseowner"
        ...
        if alvo then
            ...
            -- verde = agindo agora. Cinza = tem alvo, mas so age quando o
            -- pet entrar na mao.
            if fa.roubando then
    ...
    local function faceAwayLigar()
        ...
        fa.conn = RS.Heartbeat:Connect(function()
            ...
            -- LE O ATRIBUTO TODO FRAME, como no script original. Confiar so
            -- no GetAttributeChangedSignal falha quando o Stealing ja estava
            -- ativo antes do modulo subir -- o sinal nunca dispara e o
            -- recurso fica morto ate voce largar e pegar o pet de novo.
            ...
            if roubando ~= fa.roubando then
                ...
                if not roubando then
                    soltarRotacao()
                    fa.travado = nil          -- largou o pet: destrava
                end
            end

            -- quem e o alvo nao muda a cada frame: reavaliar 4x/s basta e
            -- evita varrer os Plots 240 vezes por segundo
            local agora = os.clock()
            ...
            -- a rotacao roda TODO frame enquanto o pet esta na mao: o alvo
            -- anda, e as costas precisam acompanhar
    ...
    -- ========================================================
    --  4. ANTI COLISAO
    --  Atravessar players e clones deles.
    --
    --  E client-side: a fisica do SEU personagem roda na sua
    --  maquina, entao zerar CanCollide nas partes dos outros
    --  basta pra voce passar por dentro. Na tela deles nada muda.
    --
    --  O proprio personagem fica de fora sempre. Zerar CanCollide
    --  em si mesmo faz cair pelo mapa.
    -- ========================================================
    local acConns = {}

    local function passarPor(modelo)
        if not modelo then return end
        if modelo == eu.Character then return end          -- nunca em si mesmo
    ...
    local function antiColisaoLigar()
        ...
        -- clones e NPCs: qualquer Model com Humanoid que nao seja voce
        for _, m in ipairs(workspace:GetDescendants()) do
        ...
        acConns[#acConns+1] = workspace.DescendantAdded:Connect(function(d)
            ...
            if d:IsA("BasePart") then
                -- parte nova dentro de alguem que ja esta liberado
    ...
    local function antiColisaoDesligar()
        ...
        table.clear(acConns)
        -- a colisao volta sozinha no proximo respawn de cada um; forcar
        -- CanCollide=true aqui religaria partes que o jogo quer sem colisao
        -- (acessorios, hitbox de efeito), entao nao mexemos
    end
print("free hub discord.gg/sabcom")
    -- ========================================================
    --  API que a aba UI e os keybinds usam
    -- ========================================================
    _G.sabcomExtras = {
        ...
        setFaceNearest = function(on)
            ...
            -- o toggle/keybind so alterna entre esses dois. Escolher uma
            -- pessoa especifica e coisa da lista do painel.
            Config.FaceAwayModo = Config.FaceAwayNearest and "nearest" or "baseowner"
            Config.FaceAwayAlvo = nil
            setToggle("Face Nearest", Config.FaceAwayNearest)
            fa.travado   = nil        -- troca de modo zera o alvo travado
            fa.alvoCache = nil
            marcaLimpar()
            pcall(pintarSelecao)
            pcall(saveConfig)
...
print("free hub discord.gg/sabcom")
    -- registra os toggles pra UI e o setToggle funcionarem
    pcall(function()
...
print("free hub discord.gg/sabcom")
print("free hub discord.gg/sabcom")
    -- restaura o que estava ligado na sessao anterior
    task.spawn(function()
        task.wait(2)
        pcall(construirBotaoPs)       -- botao flutuante sempre visivel
